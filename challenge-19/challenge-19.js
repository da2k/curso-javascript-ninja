( function(){
    ...
    */
    ...
    var regex = new RegExp('<\\w*\\s*class=.(\\w*)\\s*?(\\w*).>', 'g');
    var resMatch = markup.match(regex);
    var resTest = regex.test(markup);
    ...
    function hasClass(markup, cssClass) {
        if (regex.test(cssClass) === true) {
            console.log("Valid");
        } else {
            console.log(markup, cssClass)
            console.log("Invalid");
        }
    }
