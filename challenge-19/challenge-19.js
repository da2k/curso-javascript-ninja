...
function hasClass (markup, cssClass){
    var regex = new RegExp('class=["\'](?:[\\w\\s]+)?' + cssClass + '(?:[\\w\\s]+)?["\']', 'gmi')
    return regex.test(markup)
}

var classArray = ['container', 'text', 'date', 'excerpt', 'main']
classArray.forEach(function(classArray){
    console.log(`${hasClass(markup, classArray)} para a classe ${classArray}`)
})
