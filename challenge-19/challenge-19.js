...
(function(){
    ...
    */
    ...
    function hasClass( markup, cssClass ) {
        var regex = new RegExp( 'class=[\'\"]\\w*?\\s?(' + cssClass + ')\\s?\\w*?[\'\"]', 'gm' );

        console.log( regex.test(markup) + ' para a classe ' + cssClass);
    }
    hasClass(markup, 'container');
    hasClass(markup, 'text');
    hasClass(markup, 'date');
    hasClass(markup, 'excerpt');
    hasClass(markup, 'main');
