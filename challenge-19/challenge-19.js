(function () {
	...
	*/
	...
	var firstsNumbers = new RegExp('(^\\d+)', 'gm');
	console.log(firstsNumbers);
	...
	*/
	...
	console.log(text.match(firstsNumbers));
	...
	*/
	...
	var finalNumbers = new RegExp('(\\d+$)', 'gm');
	...
	*/
	...
	console.log(otherText.match(finalNumbers));
	...
	*/
	...
	var classRegex = new RegExp('(\\<\\w+ )(class=[\'\"])(.+)([\'\"].+)', 'gm');
	console.log('\n' + classRegex);
	console.log( '\nQuais classes CSS existem na marcação abaixo?\n\n', markup, '\n' );
	function hasClass (markup, cssClass) {
		var result = 0;
		var phrase = ' para a classe ' + cssClass;
		markup.replace(classRegex, function (regex, openTag, classAtribute, className) {
			return (className.indexOf(cssClass) >= 0) ? result = 1 : '';
		});
		return (result) ? true + phrase : false + phrase;
	}
