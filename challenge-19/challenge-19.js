(function(){
    ...
    */
    ...
    function hasClass(markup, cssClass){
        var regEx = /(?:\'|\"|\\'|\\")\w+(?:\s\w+)*(?:\'|\"|\\'|\\")/g;
        var tratamento = markup.match(regEx);
        
        //'tratamento' possui strings com algumas aspas, então eu as removo uma por uma.
        tratamento = tratamento.map(function(value){
            return value.replace(/['"]/g, '');
        });

        //'tratamento' possui 'text' e 'date' juntos no mesmo espaço, então eu junto tudo numa string e depois separo todos;
        tratamento = tratamento.join(' ');
        tratamento = tratamento.split(' ');

        return tratamento.indexOf(cssClass) > -1;
    }

    var cssClasse = 'container';
    console.log(hasClass(markup, cssClasse) + ' para a classe ' + cssClasse);
