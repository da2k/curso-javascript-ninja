(function(){
    ...
    */
    ...
    function hasClass( markup, cssClass ) { 
        var newRegexp = new RegExp('class=["\'](?:[\\w\\s]+)?' + cssClass + '(?:[\\w\\s]+)?["\']');
        return newRegexp.test( markup );
    }

    var allClasses = ['container', 'text', 'date', 'excerpt', 'main' ]; 

    allClasses.forEach(function( singleClass ){
        console.log(hasClass(markup, singleClass) + ' para a classe ' + singleClass );
    });
