(function(){
  ...
  */
  ...
  function hasClass(markup, cssClass) {
    cssClass = new RegExp('class=("|\')'+cssClass+'(\\1)','gmi');
    return markup.search( cssClass ) === -1 ? false : true;
  }

  function printMessage ( classe ) {
    console.log( hasClass(markup, classe) + ' para a classe ' + classe );
  }
  printMessage( 'container' );
  printMessage( 'text' );
  printMessage( 'date' );
  printMessage( 'excerpt' );
  printMessage( 'main' );
