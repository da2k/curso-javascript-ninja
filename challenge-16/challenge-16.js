(function () {
    ...
    */
    console.log('\nMeus amigos:');
    var amigos = ['abreu', 'abravanel', 'abel', 'abelardo', 'arnaldo'];
    var result = '';
    amigos = amigos.forEach(function (item, index) {
        if (index !== amigos.length - 1) {
            result += item.charAt(0).toUpperCase().concat(item.slice(1)) + ', ';
        } else {
            result = result.slice(0, result.length - 2);
            result += ' e ' + item.charAt(0).toUpperCase().concat(item.slice(1)) + ' são meus amigos.';
        }
    });
    console.log(result);
