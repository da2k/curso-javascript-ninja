...
while( c < name.length) {
    console.log(`${name.charAt(c)} é a ${++c}ª letra do meu nome`)
}
...
var fullName ='raphael-silvestre'
console.log(fullName)
var arrayNome = fullName.split('-').map(function (name){
...
var frase = nomes.reduce(function (acumulado, atual , index){
    var separador = nomes.lenght - 1 === index ? ' e ' : ' , '
...
console.log( '\nEra "Roberto", agora é:' );
var name = 'Roberto'
var trocaNome = name.replace('to','ta')
...
for( var i = 0 , len = myName.length ; i < len ; i++) {
    newName2.push( i % 2 === 0 ? myName[i].toLowerCase() : myName[i].toUpperCase())
}
