(function() {
  ...
  */
  ...
  var fullName = 'natan-siqueira-alves';
  var fullNameResult = '';

  for( var i = 0; i < fullName.split( '-' ).length; i++ ) {
    fullNameResult += fullName.split( '-' )[ i ].charAt( 0 ).toUpperCase() + fullName.split( '-' )[ i ].substring(1) + ' ';
  }
  ...
  */
  console.log( '\nMeus amigos:' );
  var names = [ 'Júlia', 'Gabriela', 'Ana', 'André', 'Pedro' ];

  var reduce = names.reduce( function( acumulado, atual, index ) {
    return names.length === ( index + 1 ) ? acumulado + ' e ' + atual : acumulado + ', ' + atual;
  } );

  var friends = reduce.concat( ' são meus amigos.' );
  ...
  */
  console.log( '\nEra "Roberto", agora é:' );
  console.log( 'Roberto'.replace( 'berto', 'berta' ) );
