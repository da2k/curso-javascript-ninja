(function(win, doc) {
  ...
  */
  ...
  var $buttonsNumbers = doc.querySelectorAll('[data-js="number"]');
  var $buttonsOperations = doc.querySelectorAll('[data-js="op"]');
  var $buttonEqual = doc.querySelector('[data-js="equal"]');
  var $buttonClear = doc.querySelector('[data-js="clear"]');

  Array.prototype.forEach.call($buttonsNumbers, function(number) {
    number.addEventListener( 'click', addNumber , false );
  });

  Array.prototype.forEach.call($buttonsOperations, function(operation) {
    operation.addEventListener( 'click', addOperation, false );
  });

  $buttonEqual.addEventListener( 'click', result, false );
  $buttonClear.addEventListener( 'click', clearVisor, false );
  ...
  function result() {
    ...
    $visor.value = allValues.reduce(function(total, currentValue) {
      var firstValue = total.slice(0, -1);
      var operator = total.split('').pop();
      var lastValue = removeLastItemIfIsAnOperator(currentValue);
