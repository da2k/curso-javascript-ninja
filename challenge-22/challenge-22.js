(function () {
  ...
  */
  ...
  Object.keys(names).map(function (key, index) {
    var arr = names[key];
    console.log(getFullName.call(arr));
  });
  ...
  */
  ...
  var objNumbers = {
    arrNumber1: [3, 1, 21],
    arrNumber2: [821, 21, 65, 98],
    arrNumber3: [54, 5, 132, 856, 4]
  }

  Object.keys(objNumbers).map(function (key, index) {
    var arr = objNumbers[key];
    console.log(sum.apply(sum, arr));
  });

  //arrNumbers.map(function (item, index) {
  // console.log(sum.apply(sum, item));
  //});
