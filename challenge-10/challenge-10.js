(function(){
    ...
    */
    //acredito que esta não seja a melhor solução, mas não consegui pensar em outra sem usar "if" ou "switch"
    function isOperatorValid(op){
        var resp = op === '+' ? true : (op === '-' ? true : (op === '*' ? true : (op === '/' ? true : (op === '%' ? true : false))));
        return resp;
    }
    ...
    */
    operationSignal = '$';
    var sinalErrado = calculator(operationSignal);
    if(sinalErrado){
        number1 = 37;
        number2 = 7;
        console.log(showOperationMessage(operationSignal, number1, number2), sinalErrado(number1, number2));
    }else{
        console.log(showErrorMessage(operationSignal));
    }
