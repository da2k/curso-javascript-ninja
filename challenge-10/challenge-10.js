(function (){
    ...
    */
    ...
    /*
    Sem alterar os códigos nos `console.log` abaixo, faça com que o retorno deles seja "true", usando os Wrapper Objects como "conversores" nos valores das variáveis. Analise o que está sendo impresso no console para saber como resolver o problema corretamente.
    */
    var five = '5';
    five = Number (five);
    ...
    var concat = 10 + 10;
    // Eu havia entendido que não tinha a liberdade de intervir nessa linha, mas que deveria incluir algum comando entre as a declaração da var que você nos deu e o console que vc também nos deu:
    ...
    - Cada propriedade vai receber uma função (logo, elas serão métodos), e essa função receberá dois parâmetros e retornará a operação referente à sua propriedade, usando os valores passados por parâmetro.
    */
    ...
    // PROFESSOR:
    // sem IF , sem SWITCH, sem TERNARY
    function isOperatorValid( op )  {
        return (op === '+' || op === '-' || op === '*' || op === '/' || op === '%');
    }
    
    // MELHORADO, MAIS DINÂMICO:
    function isOperatorValid( op )  {
    ...
    - Senão, retornar o método do objeto "operation" criado acima, baseado no operador passado para a função "calculator", e passando para esse método os dois parâmetros da função de retorno de "calculator".
    */
    ...
    // PROCURE EVITAR UTILIZAR 'ELSE'. Use o mínimo possível de IF e SWITCH para ter maior legibilidade e ter responsabilidades mais específicas para cada função. Cada função deve ter a sua única responsabilidade.
    ...
    */
    ...
    // ATENÇÃO: Considerar que espaços vazios no console não são descartados. Ou seja, acima poderíamos ter aproveitado para usar espaços dentro do próprio console, quando oportuno, dispensando espaços nas operações de concatenação de strings.
    ...
    */
    ...
    operationSignal = 'ALGUM ERRO AQUI';
    var mod = calculator(operationSignal);
    if (mod) {
        number1 = 35;
        number2 = 6;
