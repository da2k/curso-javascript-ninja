(function() {
  ...
  */
  ...
  function step2(operationSignal, operation, number1, number2) {
    if(isOperatorValid(operationSignal)) {
      var result = operation(number1, number2);
      if(typeof(result) === 'number') {
        var message = showOperationMessage(operationSignal, number1, number2);
        console.log(message, result);
      }
    } else {
      var errorMessage = showErrorMessage(operationSignal);
      console.log(errorMessage);
    }
  }

  step2(operationSignal, sum, number1, number2);
  /*
  ...
  */
  ...
  step2(operationSignal, subtraction, number1, number2);


  operationSignal = '*';
  var multiplication = calculator(operationSignal);
  step2(operationSignal, multiplication, number1, number2);

  operationSignal = '/';
  var division = calculator(operationSignal);
  step2(operationSignal, division, number1, number2);

  operationSignal = '%';
  var mod = calculator(operationSignal);
  step2(operationSignal, mod, number1, number2);
  ...
  */
  var invalidOperationSignal = '%%';
  var invalidOperation = calculator(invalidOperationSignal);
  step2(invalidOperationSignal, invalidOperation, number1, number2);
