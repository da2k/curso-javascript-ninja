...
(function() {
	...
	*/
	function isOperatorValid( operador ) {
		// as operações já estao em cima (operation), futuramente se adicionarmos mais, não queremos acrescentar nesta nova função
		//tem que ser automatico, dai usarmos a função de cima
		return operation[operador] !== undefined;

		//OU podemos também usar o Truphy, retornar função é truphy
		//return !!operation[operator]; //se o operador for função, ele retorna bem 
	};
		...
		if( !isOperatorValid(operador) ) {
			...
			return function ( param1 , param2){
				if( typeof param1 !== 'number' || typeof param2 !== 'number') 
					return false;
				return operation[operador ](param1 , param2 ); //usar formato de array pois nao sei o que vai ser recebido
			};
