(function(){
    ...
    */
    ...
    console.log( '"' + concat + '" é uma string? E é igual a "1010"?', typeof concat === 'string' ); // "1010" é uma string? E é igual a "1010"? true
    ...
    */
    ...
    function isOperatorValid(op){
        let validArray=['+', '-', '*', '/','%'];
        return validArray.indexOf(op)>-1 ? true : false;
    }
    ...
    */
    //
    function calculatorResult(result, signal){
        let errorMessage = result === false ? showErrorMessage(signal) : '';
    ...
    calculatorResult(sum, operationSignal);
    ...
    */
    ...
    operationMap.set(operationSignal, subtraction);
    
    
    operationSignal = '/';
    let division = calculator(operationSignal);
    operationMap.set(operationSignal, division);
    
    
    operationSignal = '*';
    let multiplication = calculator(operationSignal);
    operationMap.set(operationSignal, multiplication);
    
    
    operationSignal = '%';
    let mod = calculator(operationSignal);
    operationMap.set(operationSignal, mod);

    operationMap.forEach((value,key)=>{
        calculatorResult(value,key);
    })
    /*
      A operação 23 - 57 = -34
      A operação 23 / 57 = 0.40350877192982454
      A operação 23 * 57 = 1311
      A operação 23 % 57 = 23
    */
    ...
    */
    ...
    calculatorResult(invalid, operationSignal);
