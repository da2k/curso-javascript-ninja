( function () {
	...
	*/
	...
	function calculator (operator) {
		...
		if (isOperatorValid(operator)) {
			...
			return function (x,y) { 
				return (typeof x === 'number' && typeof y === 'number' ? operation[operator](x,y) : false );
			};
	...
	*/
	...
	console.log( sum(number1,number2) ? showOperationMessage(operationSignal, number1, number2) + sum(number1,number2) : showErrorMessage() );
	/*
	...
	*/
	...
	console.log( subtraction(number1,number2) ? showOperationMessage(operationSignal, number1, number2) + subtraction(number1,number2) : showErrorMessage() );
	
	operationSignal = '*';
	var multiplication = calculator(operationSignal);
	console.log( multiplication(number1,number2) ? showOperationMessage(operationSignal, number1, number2) + multiplication(number1,number2) : showErrorMessage() );

	operationSignal = '/';
	var division = calculator(operationSignal);
	console.log( division(number1,number2) ? showOperationMessage(operationSignal, number1, number2) + division(number1,number2) : showErrorMessage() );

	operationSignal = '%';
	var mod = calculator(operationSignal);
	console.log( mod(number1,number2) ? showOperationMessage(operationSignal, number1, number2) + mod(number1,number2) : showErrorMessage() );
