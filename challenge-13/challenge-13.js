(function(){
	...
	*/
	function checkLength(state, index){
		return state.length > 7;
	}
	var result = brasil.every(checkLength);
	...
	*/
	function includesCeara(state, index) {
		return state === 'Ceará';
	}
	result = brasil.some(includesCeara);
	...
	*/
	var map = newBrasil.map(function(item){
		return {
			id: item.id + 1,
			state: item.estado + ' pertence ao Brasil.'
		};
	});
