...
console.log([ 0 , 1 , 2 , 3 , 4 , 5].toString())
...
var nordeste = ['Pernambuco', 'Alagoas', 'Paraiba', 'Sergipe', 'Rio Grande do Norte', 'Bahia', 'Piaui', 'Ceara', 'Maranhão']
...
Brasil.forEach(function (item) {
    if(item.length > 7) {
        testeLetras.push(item)
    }
})
console.log(testeLetras)
...
var filter = map.filter(function(item){
    return item.id % 2 === 0 // A função filter ja retorna o item se a condição for true
})
