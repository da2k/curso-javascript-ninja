(function() {
	...
	*/
	...
	var verify = brasil.every( function( estado ) {
	...
	*/
	...
	var verifyState = brasil.some( function( estado ) {
		return estado === 'Ceará';
	});
	console.log( verifyState ? 'Ceará está incluído!' : 'Ceará não foi incluído :(' );
	...
	*/
	var mapping = newBrasil.map( function( state, id ) {
		return {
			id: state.id++,
	...
	*/
	var filter = mapping.filter( function( state, id ) {
		return state.id % 2 === 0;
	});
