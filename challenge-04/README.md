...
carro.addPessoas = function(qtd){
  ...
  var pessoasQtd = vagas === 1 ? 'pessoa' : 'pessoas';

  if(carro.quantidadePessoas === carro.assentos &&  qtd > 0){
    return 'O carro já está lotado!';
  }

  if(carro.quantidadePessoas + qtd  > carro.assentos ){
    return 'Só cabem mais '+vagas+' '+pessoasQtd+'!';
  }

  if (qtd < 0 && (qtd*-1) > carro.quantidadePessoas) {
      return 'So tem '+carro.quantidadePessoas +' '+ pessoasQtd + ' no carro.';
  }
