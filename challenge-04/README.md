...
carro.addPessoas = function(num){
    ...
    if(carro.quantidadePessoas === carro.assentos && totalPessoas >= carro.assentos){
        return 'O carro já esta cheio!';
    };
    if (totalPessoas > carro.assentos){
        var assentosLivres = carro.assentos - carro.quantidadePessoas;
        if(assentosLivres === 1){
            return 'Só cabem mais ' + assentosLivres + ' pessoa no carro';
        }
        else {
            return 'Só cabem mais ' + assentosLivres + ' pessoas no carro';
    }
    }
...
carro.obterMarcaModelo(); // "Este carro é um Chevrolet Celta"
