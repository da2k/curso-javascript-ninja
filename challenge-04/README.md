...
*/
carro.adicionarPessoas= function (numerosPessoas) {
 var totalpessoas = carro.quantidadesPessoas + numerosPessoas;
 
 if( carro.quantidadesPessoas === carro.assentos &&  totalpessoas >= carro.assentos ) {
 return 'O carro já esta lotado!';
     }
  if( totalpessoas > carro.assentos ){ 																
  	var vagaPessoas = carro.assentos - carro.quantidadesPessoas;					                  
    var plural = vagaPessoas === 1 ? 'pessoa' : 'pessoas';											 

      return 'Só cabem mais ' + vagaPessoas +  plural + '!';        
       }																							
     carro.quantidadesPessoas += numerosPessoas;	   
...
carro.obterMarcaModelo(); //"Esse carro é um Porsche 911 Turbo S."
