...
isTruthy(2 / 2);
isTruthy('will');
isTruthy({ nome: 'willian' });
isTruthy(['banana', 'uva', 'pera']);
isTruthy(function isTruthy() {});
...
carro.adicionarPessoas = function(numPessoas) {
    var qtdPessoas = carro.quantidadePessoas + numPessoas,
        restantes  = carro.assentos - carro.quantidadePessoas,
        fechaFrase = (restantes > 1) ? 's!' : '!'; 

    if(carro.quantidadePessoas === carro.assentos && qtdPessoas >= carro.assentos) 
        return 'O carro já está lotado!';

    if(qtdPessoas > carro.assentos) 
        return 'Só cabem mais ' + restantes + ' pessoa' + fechaFrase;
