...
carro.adicionarPessoas = function(quantidade) {
	var pessoas = this.quantidadeDePessoas + quantidade;
	var pluralPessoas = 'pessoas';

	if(this.quantidadeDePessoas === this.assentos && pessoas >= this.assentos) {
		return `O carro já está lotado!`;
	}
	
	if(this.quantidadeDePessoas === (this.assentos - 1)) {
		pluralPessoas = 'pessoa';
	}

	if(pessoas > this.assentos) {
		return `Só cabem mais ${ this.assentos - this.quantidadeDePessoas } ${pluralPessoas}`
	}
