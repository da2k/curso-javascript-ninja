...
var isTruthy = function (param) {
  return param ? true : false; //return !!param;
};
...
carro.adicionarPessoas = function (quantidade) {
  ...
  var assentosLivresPlural = assentosLivres > 1 ? "pessoas" : "pessoa";

  if (
    carro.assentos === carro.quantidadePessoas &&
    quantidade >= carro.assentos
  ) {
    return "O carro já está lotado!";
  }

  if (quantidade > assentosLivres) {
    return `Só cabem mais ${assentosLivres} ${assentosLivresPlural}!`;
  }
  carro.quantidadePessoas += quantidade;
  var quantidadePessoasPlural =
    carro.quantidadePessoas > 1 ? "pessoas" : "pessoa";

  return `Já temos ${carro.quantidadePessoas} ${quantidadePessoasPlural} no carro!`;
};
...
carro.mudarCor("preto"); //undefined
