...
*/
carro.adicionarPessoa = (qtd) => {
  if(qtd > 0 && carro.assentos <= carro.quantidadePessoas) {
    return 'O carro já está lotado!';
  }
  if(carro.assentos < (carro.quantidadePessoas + qtd)) {
    var disponivel = carro.assentos - carro.quantidadePessoas;
    var pessoas = (disponivel === 1)?'pessoa':'pessoas';
    return `Só cabem mais ${disponivel} ${pessoas}!`;
  }
  if(carro.quantidadePessoas + qtd < 0) {
    qtd = -1*carro.quantidadePessoas;
  }
