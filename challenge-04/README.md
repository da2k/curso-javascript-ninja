...
para o contrário.
*/
var isTruthy = function(par) { return par ? 'true' : 'false'}
...
isTruthy('0');
isTruthy([1,2,3,4,5]);
isTruthy({nome: Hugo, sobrenome: Sousa});
isTruthy(true);
...
carro.adicionarPessoas = function(numPessoas) {
      ...
      var diferença = carro.assentos - carro.quantidadePessoas;
      var pessoa = ' pessoas';
      var caber = diferença > 1 ? 'cabem' : 'cabe'
      //var retorno = 'Só ' + caber + ' mais ' + numPessoas + ' ' + pessoa + '!';

      if (carro.quantidadePessoas >= carro.assentos && numPessoas > 0) {
        return 'O carro já está lotado!'
      }

      else if (carro.quantidadePessoas < carro.assentos && (carro.quantidadePessoas + numPessoas) > carro.assentos) {
        return 'Só ' + caber + ' mais ' + diferença + pessoa + '!';
      }
