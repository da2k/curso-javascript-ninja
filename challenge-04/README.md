...
carro.adicionarPessoas = function(num) {
    ...
    var temVaga = carro.quantidadePessoas < carro.assentos;
    ...
    if (temVaga && totalPessoas > carro.assentos) {
...
carro.removerPessoas = function(num) {

    if (num > 0 && (num <= carro.assentos || num > carro.assentos) ) {

        if (num > carro.quantidadePessoas) {

            return 'O número de pessoas a serem removidas não pode ser maior que a quantidade de pessoas do carro';
        }
        ...
        return 'O carro agora tem ' + carro.quantidadePessoas + ' ' + pessoa;
    }
};
