...
isTruthy( 10 + 10 );
isTruthy( [ 1, 2, 3 ] );
isTruthy( { name: 'Luke', lastName: 'SkyWalker' } );
isTruthy( 42 );
isTruthy( true );
...
carro.adicionarPessoas = function( pessoas ) {
  ...
  if( carro.quantidadePessoas < 5 && totalPessoas > 5 ) {
    var singularOuPlural = carro.quantidadePessoas === 4 ? 'pessoa' : 'pessoas';  
    return 'Só cabem mais ' + (carro.assentos - carro.quantidadePessoas) + ' ' + singularOuPlural + '!';
  }
...
carro.obterMarcaModelo(); // "Esse carro é um Hyundai IX-35" 
