...
let total = this.quantidadePessoas + numPessoas;
let vagas = this.assentos - this.quantidadePessoas;
let pessoas = vagas === 1 ? 'pessoa' : 'pessoas';
let cabem = this.quantidadePessoas === 4 ? 'cabe' : 'cabem';
  if (this.quantidadePessoas === this.assentos && total >= this.assentos) {
    return `O carro já está lotado!`;
  } else if (total > this.assentos) {
