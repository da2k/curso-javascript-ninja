...
carro.addPessoa = function(quantidade) {
  ...
  if(quantidade > assentosRestantes) {
    var pessoa = assentosRestantes > 1 ? 'pessoas' : 'pessoa';
    return 'Só cabem mais ' + assentosRestantes + ' ' + pessoa + '!';
  }
  
  carro.quantidadePessoas += quantidade;
  
  var pessoa = carro.quantidadePessoas > 1 ? 'pessoas' : 'pessoa';
...
carro.obterMarcaModelo(); // Esse carro é Chevrolet Cruze
...
// Faça o carro encher.
carro.addPessoa(1); // Já temos 3 pessoas no carro!
carro.addPessoa(1); // Já temos 4 pessoas no carro!
carro.addPessoa(1); // Já temos 4 pessoas no carro!
