...
para o contrário.
*/
var isTruthy = function(parametro1) {
	if (parametro1)
		return true;
	return false;
}
...
carro.addPessoas = function(numPessoas) {
	...
	var pessoaSingular = "pessoa";
	if (carro.quantidadePessoas >= NUMERO_MAXIMO) {
		return "O carro já está lotado.";
	} else if (numPessoas > (NUMERO_MAXIMO - carro.quantidadePessoas)) {
		if (NUMERO_MAXIMO - carro.quantidadePessoas > 1) {
			pessoaSingular+= "s";
		}
		return "Só cabem mais " + (NUMERO_MAXIMO - carro.quantidadePessoas) + " " + pessoaSingular + "!";
	} else {
...
carro.obterMarcaModelo(); // Esse carro é um Volkswagen Gol G5
