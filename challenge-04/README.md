...
isTruthy({});
isTruthy(function() {});
isTruthy('Bodybuilding');
isTruthy('Ninja');
isTruthy(20 * 10);
isTruthy('Javascript Coder');
...
*/
carro.obterMarcaModelo = function(marca, modelo) {
...
carro.adicionarPessoas = function(numPessoas) {
    ...
    if( total > carro.assentos ) {
        ...
        var alteraSintaxe = vagasRestantes === 1 ? ' pessoa' : ' pessoas';
        return `Só cabem mais ${vagasRestantes} ${alteraSintaxe}`;
    }
...
carro.obterMarcaModelo(); // "Esse carro é um Ford Fiesta Sedan"
