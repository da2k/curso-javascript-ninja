...
carro.adicionarPessoas = function(x) {
  var vagas = carro.assentos - carro.quantidadeDePessoas;
  flexaoPessoa = vagas === 1 ? "pessoa" : "pessoas";

  if (x < 0) {
    if (vagas === carro.assentos) {
      return "Não é possível remover ninguém pois o carro está vazio";
    }
    if (-x > carro.quantidadeDePessoas) {
      return (
        "Você pode remover no máximo " +
    ...
    return "Só cabem mais " + vagas + " " + flexaoPessoa + "!";
  } else {
    carro.quantidadeDePessoas += x;
    return "Já temos " + carro.quantidadeDePessoas + " pessoas no carro!";
  }
