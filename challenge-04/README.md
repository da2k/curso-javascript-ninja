...
para o contrário.
*/
var isTruthy = function(value) { return value ? 'true' : 'false' }
...
*/
carro.mudarCor = function(cor) { this.cor = cor }
...
carro.adicionarPessoas = function(numero) {
    ...
    if (this.quantidadePessoas === this.assentos) {
        return 'Carro lotado';
    } else if (numero > (this.assentos - this.quantidadePessoas)) {
        var s = (this.assentos - this.quantidadePessoas) > 1 ? 's' : '';
        var m = (this.assentos - this.quantidadePessoas) > 1 ? 'm' : '';
        return 'Cabe' + m + ' apenas ' + (this.assentos - this.quantidadePessoas) + ' pessoa' + s;
    } else {
        this.quantidadePessoas += numero;
        var p = this.quantidadePessoas > 1 ? 's' : '';
        return "Já temos " + this.quantidadePessoas + " pessoa" + p;
    }
}
...
carro.adicionarPessoas(4); //Cabem apenas 3 pessoas
...
carro.adicionarPessoas(10); //Cabem apenas 4 pessoas
