...
var pluralCabem= quantasPessoasCabem === 1 ? 'cabe' : 'cabem';
//Adicionando mais uma condição para conseguir fazer a retirada de pessoas.
//só entrará no primeiro if se a qtde pessoas  = 5 e total for >= 5, não entrando aqui ele calcula minha retirada no final.

    if(carro.quantidadePessoas === carro.assentos && total >= carro.assentos){
        return 'O carro já está lotado!';
    }
    if(total > carro.assentos){
        return 'Só '+pluralCabem+' '+quantasPessoasCabem+' '+pluralSingular+'!';
    }
...
/*carro.addPessoas= function (numPessoas){
    ...
    if(numPessoas <= carro.assentos){
        ...
        return 'Já temos '+carro.quantidadePessoas+' pessoas no carro!';
    }else if(carro.assentos > 0 && carro.assentos < 5){
        return 'Só cabem mais '+carro.assentos+' '+passageiro+'!';
    }else{
        if(carro.quantidadePessoas === 5){
            return 'O carro está lotado!';
        }
        return 'Ultrapassou o limite do carro.';
    }   
