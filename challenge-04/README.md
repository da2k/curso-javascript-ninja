...
*/
carro.addPessoas = function(addPessoas) {
  var totalPessoas = carro.quantidadePessoas + addPessoas;
  var pessoasRestantes = carro.assentos - carro.quantidadePessoas;
  var pessoas = pessoasRestantes === 1 ? 'pessoa' : 'pessoas';
  var pessoas = carro.quantidadePessoas === 1 ? 'pessoa' : 'pessoas';
  var cabem = pessoasRestantes === 1 ? 'cabe' : 'cabem';

  if (carro.quantidadePessoas === carro.assentos && totalPessoas >= carro.assentos) {
    return `O carro já está lotado!`;
  } else if (totalPessoas > carro.assentos) {
    return `Só ${cabem} mais ${pessoasRestantes} ${pessoas}`;
  }
