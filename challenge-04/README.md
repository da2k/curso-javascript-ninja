...
carro.adicionarPessoas = function(nPessoas) {
  var tPessoas = carro.quantidadePessoas + nPessoas;
  if(carro.quantidadePessoas === carro.assentos && tPessoas >= carro.assentos) {
    return 'O carro já está lodato';
  };
  if(tPessoas > carro.assentos) {
    var qPessoasTotal = carro.assentos - carro.quantidadePessoas;
    var pluPessoas = qPessoasTotal === 1 ? 'pessoa' : 'pessoas';
    return 'Só cabem mais ' + qPessoasTotal + ' ' + pluPessoas + '!'
  };
