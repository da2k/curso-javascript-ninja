...
var troca_de_nome = text.replace(/Manuel Marques de Sousa/g, 'Vitor lima '); 

console.log(troca_de_nome); 
...
var mudar_de_cidade = text.replace(/brasileiro/g , 'Manaura'); 

console.log(mudar_de_cidade); 
...
var substituir_numeros_por_traços = text.replace(/\d/g , '-');

console.log(substituir_numeros_por_traços); 
...
var substituir_letras_por_numeros_zero = text.replace(/[D-Hd-h]/g , "0" ); 

console.log(substituir_letras_por_numeros_zero); 
/*
...
var sub_a_por_numero_qutro = text.replace(/a/ig , 4 ); 

console.log(sub_a_por_numero_qutro); 

/*
Substitua a frase "O Centauro de Luvas", deixando-a em caixa alta, usando
o método `toUpperCase()`. Mostre o resultado no console:
*/
console.log( '\n"O Centauro de Luvas" em caixa alta:' ); // precisa ser no console.log()
...
`getMonthNumber`,to correspondente a esse mês.
...
function getMonthNumber(name_Month){ 
  name_Month === 'março' ? '03': ''; 
  var months = { 
    janeiro: '01', 
    fevereiro: '02',
...
console.log(`O mês de setembro é representado pelo número ${setembro}.`);
console.log(`O mês de dezembro é representado pelo número ${dezembro}.`);
...
var regex_date = /(\d\d) de (junho|julho) de (\d\d\d\d)/g; 
...
function replaceDate(regex , day , moth , year ){
  return  `${day} / ${getMonthNumber( moth )} / ${year}}`; 
}

console.log( text.replace(regex_date, replaceDate())); 
