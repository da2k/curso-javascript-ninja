(function() {
	...
	*/
	...
	var change = text.replace(/(Manuel Marques de Sousa)/, 'Leandro Silva');
	console.log ( change );
	...
	*/
	...
	var nat = text.replace(/(brasileiro)/, 'paulista');
	console.log( nat );
	...
	*/
	...
	var sub = text.replace(/[D-Hd-h]/g, '0');
	console.log( sub );
	...
	*/
	...
	var upp = text.replace(/(O Centauro de Luvas)/, function(t, w){
  ...
  console.log('O mês de DEZEMBRO é representado pelo número', getMonthNumber('dezembro'));
	...
	*/
    ...
    return data.replace(regexDate, function (date, day, month, year){
