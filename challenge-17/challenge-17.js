(function () {
    ...
    */
    ...
    console.log(text.replace(/(Manuel Marques de Sousa)/g, 'Harry Potter'));
   ...
   console.log(text.replace(/(brasileiro)/g, 'britanica'));
    ...
    */
    ...
    console.log(text.replace(/[D-Zh-z]/g, 0));
    ...
    */
    ...
    console.log(text.replace(/(O Centauro de Luvas)/g, function (totalCapturado) {
    ...
    */
    ...
    var getMonthNumber = function (mesNome) {
        var meses = [
            {'janeiro': '01'},
            {'fevereiro': '02'},
            {'março': '03'},
        ...
        var mesNum = meses.filter(function (item, index, arr) {
            return item[mesNome];
        })[0][mesNome];
        return mesNum;
    }
    ...
    */
    ...
    var regexDate = /(\d\d)\Wde\W(junho|julho)\Wde\W(\d\d\d\d)/g
    ...
    */
    console.log( '\nReplace de datas:' );
    var replaceDate = function (fullData) {
        var newDate = '';
        var dia = (fullData.toString()).match(/\d\d/);
        var mes = (fullData.toString()).match(/junho|julho/);
        mes = getMonthNumber(mes);
        var ano = (fullData.toString()).match(/\d\d\d\d/);
        newDate = dia + '/' + mes + '/' + ano;
        return newDate;
    };
    var finalFullText = text.replace(regexDate, function (totalCapturado) {
        if(totalCapturado === '18 de julho de 1875')
            return replaceDate(text.match(regexDate)[1]);
        return replaceDate(text.match(regexDate));
    });
    console.log(finalFullText);
