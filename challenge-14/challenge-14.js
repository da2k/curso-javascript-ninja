(function(){
	...
	*/
	...
	var operation = justMod2Or3.reduce(function(accumulated, current){
		var sumOne = current + 1;
		return current * sumOne;
	}, 0);
	...
	*/
	...
	var operation2 = justMod2Or3.reduceRight(function(accumulated, current){
		var sumTwo = current + 1;
		return sumTwo * current;
	}, 0);
	...
	*/
	...
	var name = ['PA', 'PN', 'PD', 'PE', 'PR', 'PS', 'PO', 'PN'];
	var reduce = name.reduce(function( accumulated, current){
	...
	*/
	...
	console.log(
		numberObjects.lastIndexOf( { number: 2 } < -1 , 22 )
		? 'Existe um objeto { number: 2 } em numberObjects!'
		: 'Não existe um objeto { number: 2 } em numberObjects :(' 
	);
