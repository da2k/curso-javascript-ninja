(function () {
  ...
  */
  ...
  const justMod2Or3 = justNumbers.filter(
  ...
  */
  ...
  const operation = justMod2Or3.reduce((acc, number) => {
    return (acc + 1) * number;
  }, 0);
  ...
  */
  ...
  const operation2 = justMod2Or3.reduceRight((acc, number) => {
    return (acc + 1) * number;
  }, 0);
  ...
  */
  ...
  const pLanguage = name.reduce((acc, current) => {
  ...
  */
  ...
  const inversedName = name.reduceRight((acc, current) => {
    return `${acc}${current}`;
  }, "");
  ...
  */
  ...
  if (numberObjects.indexOf({ number: 2 }) > -1) {
    console.log(`Existe um objeto {number:2} em numberOjects`);
  } else {
