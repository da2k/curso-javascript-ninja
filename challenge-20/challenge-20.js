(function(win, doc) {
    ...
    */    
    ...
    $button.addEventListener('click', function(event){
        ...
        var validation = [
            $inputUsername.value ? true : 'Preencha o nome do usuário!',
            $inputEmail.value ? true : 'Preencha o e-mail!',
            $message.value ? true : 'Preencha a mensagem!',
            isValidEmail($inputEmail.value) ? true : 'Entre com um e-mail válido!'
        ];

        var getValidation = validation.filter(function(item) {
            return typeof item === 'string';
        });

        if (getValidation.length) {
            getValidation.forEach(function(item) {
                alert(item);
            })
            return;
        }
        ...
        /* if (getValidation.length) {
            alert(getValidation.reduce(function(acumulated, item) {
                return acumulated + '\n' + item;
            }));
            return;
        } */
    ...
    */    
    ...
    function isValidEmail(email) {
        var validatedEmail = /^(?:[\w+.])+@\w+\.\w{2,}(?:\.\w{2})?$/;
