    ...
    */
    ...
    $button.addEventListener('click', 
        function(event) {
            ...
            if (!$inputUsername.value || $inputUsername.value === 'Desconhecido') return alerta('Preencha o nome do usuário!');

            if (!$inputEmail.value) return alerta('Preencha o e-mail!');
            
            if (!$message.value) return alerta('Preencha a mensagem!');

            if (!isValidEmail($inputEmail.value)) return alerta('Entre com um e-mail válido!');

            if (window.confirm('Tem certeza que deseja enviar o formulário?')) return alerta('Enviado com sucesso!');
