...
var mensagem = win.prompt('Digite uma mensagem para ser enviada');
...
//3
    if(validacao)
        confirmacao = win.confirm('Tem Certeza que deseja enviar o formulário?');
    if(confirmacao)
...
var isvalidEmail = function isvalidEmail(email) {
    ...
    return email.match(/^[\w.+]+@[\w]+.\w{2,}(?:(?:.\w{0,2})?)$/gm);
}
})(window, document);