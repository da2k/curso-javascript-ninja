    ...
    */
    $button.addEventListener('click', function (event) {
        ...
        if (!$inputUsername)
          return  alert('Preecha o nome de usuário!')
        if (!$message.value)
          return  alert('Preencha a mensagem') 
        if (!$inputEmail)
          return  alert('Preencha o e-mail!')       
        if (!isValidEmail($inputEmail.value))
          return  alert('Preencher com email valido')
        if (!confirm("Tem certeza que deseja enviar as informações?"))
          return alert("Envio cancelado")
        return alert('Infomeções enviadas com sucesso')
    }, false)
