    ...
    */
    ...
    // Professor usou a maneira de curto circuito, que e incluir o & ou ||
    // var username = prompt('Qual o seu nome ?') || 'Desconhecido'; 
    // A variavel vai receber o resultado ( nome se o usuario entrar nome OU Desconhecido senao entrar nada)
    ...
    */
    // ?
    let $button = document.querySelector('[type="submit"]'); //('button')
    ...
    */
    ...
    $button.addEventListener('click', function(event){
        event.preventDefault(); // colocamos o event.preventDefault(), para que ele nao submeta o formulario
        // antes de fazer as validacoes que precisamos.
        ...
        let messages = {
            ...
            if($inputUsername.value === '' || $inputUsername.value === 'Desconhecido'){ 
                $inputUsername.value = '';              
                return alert('Preencha ' + messages.username);
            }

             // assim $inputEmail.value  === "" e do jeito abaixo da certo.
            if(!$inputEmail.value){              
              return  alert('Preencha ' + messages.email);
            }
            ...
            if($message.value === ''){
              return alert('Preencha ' + messages.message);
            }
