(function (win, doc) {
  ...
  */
  $button.addEventListener('click', function (e) {
    ...
    var isFormFullFilled = true;
    var fields = [
      { field: $inputUsername, message: 'Preencha o nome do usuário!' },
      { field: $inputEmail, message: 'Preencha o e-mail!', },
      { field: $message, message: 'Preencha a mensagem!' }
    ];

    fields.forEach(function (obj) {
      if (obj.field.value === undefined || obj.field.value === null || obj.field.value === '') {
        alert(obj.message);
        isFormFullFilled = false;
      }
    });

    if (isFormFullFilled) {
      if (!isValidEmail(fields[1].field.value)) {
