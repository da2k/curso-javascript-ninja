(function(){
	...
	*/
	...
	var regexCleanCPF = /\D/g;

	var cleanedCPF1 = '049-214 3421-1'.replace(regexCleanCPF, cleanCPF);
	var cleanedCPF2 = '210.458.522-05'.replace(regexCleanCPF, cleanCPF);
	var cleanedCPF3 = '735 500 794 - 22'.replace(regexCleanCPF, cleanCPF);
	var cleanedCPF4 = '101.123-131x32'.replace(regexCleanCPF, cleanCPF);

	console.log(cleanedCPF1);
	console.log(cleanedCPF2);
	console.log(cleanedCPF3);
	console.log(cleanedCPF4);
	/*
	...
	*/
	...
	console.log( formatCPF(cleanedCPF1) );
	console.log( formatCPF(cleanedCPF2) );
	console.log( formatCPF(cleanedCPF3) );
	console.log( formatCPF(cleanedCPF4) );
	...
	*/
	...
	var textMonths = 'Os meses de janeiro, junho e julho começam com a letra j.';
	var regexMonth = /ju\who/g;
	console.log(textMonths.match(regexMonth));
	...
	*/
	...
	var regexTags = /<\w+\s?>/g;
	console.log(text.match(regexTags));
	...
	*/
	...
	var textTagsOpenClosed = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';
	var regexTagsOpenClosed = /<\w+><\/\w+>/g;
	console.log(textTagsOpenClosed.match(regexTagsOpenClosed));
	...
	*/
	...
	var finalText = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';
	var regexFinalText = /<(\w+)>([^>]+)<\/\w+>/g;

	console.log( finalText.replace(regexFinalText, '<$1>O texto dentro da tag "$1" é "$2"</$1>\n') );
}());