(function(){
	...
	*/
	...
	var regexFormatCpf = /(\d{3})(\d{3})(\d{3})(\d{2})/g;

	cpfs.forEach(function(cpf) {
		console.log( cleanCPF(cpf).replace( regexFormatCpf, '$1.$2.$3-$4' ) );
	});
	...
	*/
	...
	var regexMonths = /ju(n|l)ho/gi;

	console.log( 'Os meses de janeiro, junho e julho começam com a letra j.'.match( regexMonths ) );
	...
	*/
	...
	var hmtlTags = '<div><section><blockquote>Texto <img /></blockquote></section></div>';

	console.log( hmtlTags.match( /<\w+>/g ) );
	...
	*/
	...
	var hmtlTags2 = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';

	console.log( hmtlTags2.match( /<\w+><\/\w+>/g ) );	
	...
	*/
	...
	var hmtlTags3 = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';

	var regexFormatHtml = /<(\w+)>([^<]+)<\/\w+>/gi;
	
	function htmlFormat( regex, tag, text ) {
		return '<' + tag + '>O texto dentro da tag "' + tag + '" é "' + text + '"</' + tag + '> \n';
	}

	console.log( hmtlTags3.replace( regexFormatHtml, htmlFormat ) );
	
	/*
	 Ou ...

		console.log( hmtlTags3.replace( regexFormatHtml, '<$1> O texto dentro da tag "$1" é "$2"</$1> \n' ) );
