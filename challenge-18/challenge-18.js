...
function formatCPF(cpf) {
    return cleanCPF( cpf ).replace( /(\d\d\d)(\d\d\d)(\d\d\d)(\d\d)/, '$1.$2.$3-$4' );
}
...
console.log( frase.match( /ju[l|n]ho/g ) );
...
console.log( marcacao.replace( /<(\w+)>([^<]+)<\/\w+>/g, '<$1> O texto dentro da tag "$1" é o texto "$2" </$1>\n') );
}());