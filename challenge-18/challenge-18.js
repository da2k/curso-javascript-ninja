(function(){
	...
	*/
	...
	var re = /(\d{3})(\d{3})(\d{3})(\d{2})/g;

	cpfs.forEach(function( cpf ){
		console.log( cleanCPF( cpf ).replace( re, '$1.$2.$3-$4') );
	});
	...
	*/
	...
	var textoHtml = '<div><section><blockquote>Texto <img /></blockquote></section></div>';
	console.log( textoHtml.match(/<\w+>/g) );
	...
	*/
	...
	var textoHtml = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';

	console.log( textoHtml.match(/<\w+><\/\w+>/g) );
	...
	*/
	...
	console.log(textoHtml.replace(re, '<$1>O texto dentro da tag "$1" é "$2"</$1>\n' ));
})();