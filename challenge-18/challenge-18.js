(function(){ 
  ...
  */
  console.log( '\nFormatando CPFs corretamente: Versão 1' );
  
  cpfs.forEach(function( cpf ){
    console.log (cleanCPF( cpf ).replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4'))
  })

  console.log( '\nFormatando CPFs corretamente: Versão 2' );
  cpfs.forEach(function( cpf ){
    console.log (cleanCPF( cpf ).replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, function cpfFormatted (regex, group1, group2, group3, group4){
  ...
  */
  ...
  console.log("Os meses de janeiro, junho e julho começam com a letra j.".match(/ju(n|l)ho/g )); //.match retorna um array;
  console.log("Os meses de janeiro, junho e julho começam com a letra j.".match(/ju[nl]ho/g ));//pode ser feito com um grupo de captura () ou com uma lista de chars [];
  ...
  */
  ...
  console.log("<div><section><blockquote>Texto <img /></blockquote></section></div>".match(/(<\w{3,10}>)/g ))
  ...
  */
  ...
  Crie uma expressão regular que faça o match com um texto existente dentro de uma tag HTML. O texto deve ser capturado e substituído por:
  ...
  */
  ...
  let innerText = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>";
 
  console.log( innerText.replace( /<(\w+)>([^<]+)<\/\w+>/g, '<$1> O texto dentro da tag "$1" é "</$2>"\n' ) );
