(function() {
	...
	*/
	...
	var replaceCPF = function replaceCPF( regex, match1, match2, match3, match4 ) {
	  return match1 + '.' + match2 + '.' + match3 + '-' + match4;
	};
	console.log( cleanCPF( '049-214 3421-1' ).replace( pattern, replaceCPF ) );
	console.log( cleanCPF( '210.458.522-05' ).replace( pattern, replaceCPF ) );
	console.log( cleanCPF( '735 500 794 - 22' ).replace( pattern, replaceCPF ) );
	console.log( cleanCPF( '101.123-131x32' ).replace( pattern, replaceCPF ) );	
	...
	*/
	...
	var patternTag = /(<(\w+)>)([^<]+)(<\/\w+>)/g;
	console.log( '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'.replace( patternTag, function( regex, match1, match2, match3, match4 ) {
	  return match1 + 'O texto dentro da tag ' + '"' + match2 + '"' + ' é ' + '"' + match3 + '"' + match4 + '\n';
	} ) );
})();	