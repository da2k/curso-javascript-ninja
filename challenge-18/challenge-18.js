(function(){
    ...
    */
    ...
    function cleanCPF( cpf ){
        var cleanedCPF = cpf.match( /([\d]+)/g ).join('');
        cleanedCPFS.push( cleanedCPF );
        return cleanedCPF;
    };
    ...
    */
    ...
    function cpfFormatting( cpf ){
        return cpf.match(/[\d]{3}|[\d]{2}/g).join('-');
    }

    cleanedCPFS.map( function( item ) {
        console.log( cpfFormatting( item ) )
    } )
    ...
    */
    ...
    var regexOne = /(h1)|(p)|(footer)|([\w^n][\wáíé\s]+)/g;
    var regexTwo = /([A-Z][\wáíé\s]+)/gm;
    var counterOne = 0;
    var counterTwo = 0;

    console.log( text.replace( /([A-Z][\wáíé\s]+)/gm, function() {
        var text2 = 'O texto dentro da tag "' + text.match( regexOne )[ counterOne ] + '" é "' + text.match( regexTwo )[ counterTwo++ ] + '"';
        counterOne += 3;
        return text2;
    }
    ) );
