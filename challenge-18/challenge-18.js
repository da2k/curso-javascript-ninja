(function() {
  ...
  */
  ...
  console.log(cleanCPF('101.123-131x32').replace(regexCPF, '$1.$2.$3-$4'));
  ...
  */
  ...
  var regexMonth = /ju.ho/g;
  console.log('Os meses de janeiro, junho e julho começam com a letra j.'.match(regexMonth));
  ...
  */
  ...
  function organizeHtmlToMatch (htmlToMatch){
    ...
    htmlToMatch = htmlToMatch.match(regexMatchHTML).sort();
    for (var i=0, len=htmlToMatch.length/2, aux=[]; i < len; i++){
      aux.push(htmlToMatch[i+(len)])
      aux.push(htmlToMatch[i]);
    }
    return aux;
  }
  console.log( organizeHtmlToMatch('<div><ul><li></li><li></li><li><span></span></li></ul></div>') );
  *///correção:
  ...
  */
  ...
  var regexToReplace = /(<\w*>)([^<>]*)(<\/\w*>)/g;
  var textToReplace = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';
  console.log(textToReplace.match(regexToReplace) );;
  console.log(
    textToReplace.replace (regexToReplace, function (regex, s1, s2, s3){
      return s1 + ' o texto dentro da tag ' + s1.match(/\w*/g)[1] + ' é "' + s2 + '"' + s3 + '\n';
      }
    )
  );
})();
