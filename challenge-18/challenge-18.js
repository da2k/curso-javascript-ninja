...
var cleanCPF = function cleanCPF(cpf) {
    var clearCpf = cpf.replace(/[^\d+]/g, function(regex){
        return regex = '' ;
    });
    var finalCPF = clearCpf.substring(0,3) + '.' +
                clearCpf.substring(3,6) + '.' +
                clearCpf.substring(6,9) + '-' +
                + clearCpf.substring(9,11);
    return finalCPF;
};

//outra forma de limpar
function cleanCPF( cpf ) {
    return cpf.replace(/\D/g, '');
}
// outra forma de colocar os pontos
cleanCPF("049-214 3421-1").replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4');
...
var matches = frase.match(/(ju\w+)/g);
