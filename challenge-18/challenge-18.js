(function () {
  ...
  */
  ...
  function cleanCPF(dirtyCpf) {
    ...
    return dirtyCpf.match(/\d/g).join('');
  ...
  */
  ...
  function formatCpf(cpf) {
    var newCpf;
    cpf.replace(regex, function (rule, prop1, prop2, prop3, prop4) {
      //console.log(prop1 + '.' + prop2 + '.' + prop3 + '-' + prop4);
      newCpf = prop1 + '.' + prop2 + '.' + prop3 + '-' + prop4;
    })
    return newCpf;
  }
  ...
  */
  ...
  var regExp = /(ju)(n|l)(ho)/g;

  console.log("Os meses de janeiro, junho e julho começam com a letra j.".match(regExp))
  ...
  */
  ...
  var htmlRegExp = /<(\w+)>/g;
  console.log("<div><section><blockquote>Texto <img /></blockquote></section></div>".match(htmlRegExp));
  ...
  */
  ...
  var emptyTagRegExp = /<\w+><\/\w+>/g
  console.log("<div><ul><li></li><li></li><li><span></span></li></ul></div>".match(emptyTagRegExp));
  ...
  */
  ...
  var RegularExp = /<(\w+\d?)>([\w+íéá\s]+)<(\/\w+\d?)>/g

  "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>".replace(RegularExp, function (regex, tag1, tag2) {

    console.log("<" + tag1 + ">" + "O texto dentro da tag" + ' ' + tag1 + ' ' + 'é' + ' ' + "\"" + tag2 + "\"" + "<" + '/' + tag1 + ">" + "\n");
