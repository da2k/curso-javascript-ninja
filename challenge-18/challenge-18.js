(function() {
    ...
    */
    ...
    cpfs.forEach(function( item ) {
        console.log(cleanCPF( item ).replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, '$1.$2.$3-$4'));
    });
    ...
    */
    ...
    console.log(phrase.match(/ju[nl]ho/));
    ...
    */
    ...
    var phrase2 = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';

    phrase2.replace(/(<\w+>)([^<]+)(<\/\w+>)/g, function replaceText(regex, tag, text, closeTag) {
        return console.log(tag + 'O texto dentro da tag "' + tag + '" é "' + text + '"' + closeTag);
    });
