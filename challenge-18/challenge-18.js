(function () {
	...
	*/
	...
	var Arraycpf = ['049-214 3421-1', '210.458.522-05', '735 500 794 - 22', '101.123-131x32'];

	function cleanCPF(cpf) {
		return cpf.match(/[(\d+)(\d+)(\d+)(\d+)]/g).join('');
	}

	Arraycpf.forEach( function(item, index){
	...
	*/
	...
	function replaceDocument(regexAll, a, b, c, d) {
		return a + '.' + b + '.' + c + '-' + d;
	}

	function documentCorrect(cpf){
		return cpf.replace(regex,replaceDocument);
	}

	Arraycpf.forEach( function(item, index){
		console.log(documentCorrect(cleanCPF(item)));
	})
	...
	*/
	...
	function textReplace(regexAll, tag, text){
		return  '<'+ tag +'>O texto dentro da tag "'+ tag +'" é "'  + text + '"</' + tag+ '>' +'\n';
	}

	console.log(text.replace(regex, textReplace));
