(function(){
    ...
    */
    ...
    //
    let regexCPF = /(\d{3})\D(\d{3})\D(\d{1,4})\D{1,3}(\d{1,2})/
    let cpfs = [
        '049-214 3421-1',
        '210.458.522-05',
        '735 500 794 - 22',
    ...
    function cleanCPF(regex,d1,d2,d3,d4){
        return d1+d2+d3+d4;
    }
    cpfs.forEach(item=>{
        console.log(item.replace(regexCPF,cleanCPF));
    })
    ...
    */
    ...
    //
    let regexFormat = /(\d{3})(\d{3})(\d{3})(\d{2})/
    function formatCPF(regex,d1,d2,d3,d4){
        return d1+'.'+d2+'.'+d3+'-'+d4;
    }
    cpfs.forEach(item=>{
        console.log((item.replace(regexCPF,cleanCPF)).replace(regexFormat,formatCPF));
    });
    ...
    */
    ...
    //
    let divText = "<div><section><blockquote>Texto <img /></blockquote></section></div>";
    let regexDiv = /<\/?\w{1,}\W?\/?>/g;

    console.log(divText.match(regexDiv));
    ...
    */
    ...
    // 
    let textDivEmpty = "<div><ul><li></li><li></li><li><span></span></li></ul></div>";
    let regexDivEmpty = /<\w{1,}><\/\w{1,}>/g
    console.log(textDivEmpty.match(regexDivEmpty));
    ...
    */
    ...
    //
    let finalRegex = /<(\w+)\/?>([^<]+)<\/\w+>/g;
    let finalText = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>";

    let finalMatch = finalText.match(finalRegex);
    
    function finalReplace(regex,str1, str2){
        return `<${str1}>O texto dentro da tag \"${str1}\" é \"${str2}\"<\/${str1}>\n`;
    }

    console.log(finalText.replace(finalRegex,finalReplace));
})()
