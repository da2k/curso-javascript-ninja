(function() {
  ...
  */
  ...
  const months = 'Os meses de janeiro, junho e julho começam com a letra j.'
  function findMonths(text) {
    const regex = /ju[nl]ho/g
    return text.match(regex)
  }
  console.log(findMonths(months))
  ...
  */
  ...
  const tags2 = '<div><ul><li></li><li></li><li><span></span></li></ul></div>'
  function findTag2(text) {
    const regex = /\<\w+\>\<\/\w+\>/g
    return text.match(regex);
  }
  console.log(findTag2(tags2))
  ...
  */
  ...
  const tags3 = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'
  function findTag3(phrase) {
    const regex = /<(\w+)>([^<]+)<(\/\w+)>/g
    return phrase.replace(regex, '<$1>O texto dentro da tag "$1" é "$2"</$1>\n')
  }
  console.log(findTag3(tags3))
})()