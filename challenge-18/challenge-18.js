...
*/
    ...
    */
    ...
    cpfs.forEach(function (el) {
        console.log(cleanCPF(el).replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, "$1.$2.$3-$4"))
    })
    ...
    */
    ...
    // ?
    let rule = /<\w+>/g
    console.log("<div><section><blockquote>Texto <img /></blockquote></section></div>".match(rule));
    /*
    ...
    */
    ...
    let phrase = "<div><ul><li></li><li></li><li><span></span></li></ul></div>";
    let ruleEmpty = /<\w+>\s?<\/\w+>/g

    console.log(phrase.match(ruleEmpty));
    // ?
    ...
    */
    ...
    // ?
    let marcation = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>"

    let expression = /<(\w+)>([^<]+)<\/\w+>/g

    console.log(marcation.replace(expression, function (reg, tag, text) {
