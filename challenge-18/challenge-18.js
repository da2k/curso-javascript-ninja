(function(){
  ...
  */
  ...
  function validaMonth( message ){
    return message.match(/ju[nl]ho/g);
  };
  console.log(validaMonth("Os meses de janeiro, junho e julho começam com a letra j."));
  ...
  */
  ...
  function validaTag( tag ){
    return tag.match(/<\w+>/g);
  };
  console.log(validaTag("<div><section><blockquote>Texto <img /></blockquote></section></div>"));
  ...
  */
  ...
  function validaTagNull( tag ){
    return tag.match(/<\w+><\/\w+>/g);
  };
  console.log(validaTagNull("<div><ul><li></li><li></li><li><span></span></li></ul></div>"));
  ...
  */
  ...
  function separator( expression ){
    return expression.replace(/<(\w+)>([^<]+)<\/\w+>/g, '<$1> O texto dentro da tag "$1" é "$2" </$1> \n');
  }
  console.log(separator("<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>"));
