(function(){
  'use strict';
	...
	*/
	...
	console.log('o CPF 049-214 3421-1 limpo fica: ' + cleanCPF('049-214 3421-1').join(''));
	console.log('o CPF 210.458.522-05 limpo fica: ' + cleanCPF('210.458.522-05').join(''));
	console.log('o CPF 735 500 794 - 22 limpo fica: ' + cleanCPF('735 500 794 - 22').join(''));
	console.log('o CPF 101.123-131x32 limpo fica: ' + cleanCPF('101.123-131x32').join(''));
	...
	*/
	...
	cpf1 = cleanCPF('049-214 3421-1').join('');
	cpf2 = cleanCPF('210.458.522-05').join('');
	cpf3 = cleanCPF('735 500 794 - 22').join('');
	cpf4 = cleanCPF('101.123-131x32').join('');
	function cpf(total, c1, c2, c3, c4){
		return c1+'.'+c2+'.'+c3+'-'+c4;
	}
	console.log(cpf1.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, cpf));
	console.log(cpf2.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, cpf));
	console.log(cpf3.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, cpf));
	console.log(cpf4.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, cpf));
	...
	*/
	...
	var arr = 'Os meses de janeiro, junho e julho começam com a letra j.'.match(/ju\w+/g);
	console.log(arr);
	...
	*/
	...
	var html = '<div><section><blockquote>Texto <img /></blockquote></section></div>'.match(/<div>|<section>|<blockquote>/g);
	console.log(html);
	/*
	...
	*/
	...
	var html2 = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';
	var div = html2.match(/(<div>|<\/div>)/g).join('');
	var li = html2.match(/((<li>)|(<\/li>))/g).join('');
	var span = html2.match(/(<span>|<\/span>)/g).join('');
	var R = [];
	R.push(div, li, span);
	console.log(R);
	...
	*/
	...
	var texto = '<h1>Título da página</h1>\n<p>Este é um parágrafo</p>\n<footer>Rodapé</footer>';
	console.log(texto);
	var texto = texto.replace(/(<h1>)(.+)(<\/h1>)/g, '<h1>O texto dentro da tag "h1" é '+ '"$2"' +'</h1>');
	console.log(texto);
	var texto = texto.replace(/(<p>)(.+)(<\/p>)/g, '<p>O texto dentro da tag "p" é ' + '"$2"' + '</p>');
	console.log(texto);
	var texto = texto.replace(/(<footer>)(.+)(<\/footer>)/g, '<footer>O texto dentro da tag "footer" é ' + '"$2"' + '</footer>');
	console.log(texto);
})();
