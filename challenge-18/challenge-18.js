(function(){
	...
	*/
	...
	function formatCpf(cpf){
		return cleanCPF(cpf).replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, function(clear, at1, at2, at3, at4 ){
	...
	*/
	...
	var textTag = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>";
	console.log( textTag.replace(/<(\w+)>([^<]+)<\/\w+>/g, '<$1>O texto dentro da tag "$1" é "$2"</$1>\n'));
