(function(){
    ...
    */
    ...
    var cpfLista = [ "049-214 3421-1", "210.458.522-05", "735 500 794 - 22", "101.123-131x32"]
    var cleanCPF = function cleanCPF( cpf ){
        return cpf.match(/\d/g).join('');
    };
    var limpos = []
    for( var i = 0; i < cpfLista.length; i++){
        limpos.push(cleanCPF(cpfLista[i]));
        console.log(cleanCPF(cpfLista[i]));
    }
    ...
    */
    ...
    for( var i = 0; i < limpos.length; i++ ){
        var first =  limpos[i].slice(0, 3);
        var second = limpos[i].slice(3, 6);
        var third = limpos[i].slice(6, 9);
        var forth = limpos[i].slice(9);
    ...
    */
    ...
    var resultado = frase.match(/ju[nl]ho/g);
    ...
    */
    ...
    var newHtmlMatch = newHtml.match(/<\w+><\/\w+>/g);
    console.log(newHtmlMatch);
    /*
