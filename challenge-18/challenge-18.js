(function () {
    ...
    */
    ...
    function cfpMask(value) {
        return value.substr(0, 3) + '.' + value.substr(3, 3) + '.' + value.substr(6, 3) + '-' + value.substr(9, 2);
    }

    function doRight(value) {
        return cfpMask(cleanCPF(value));
    }
    console.log(doRight(cpf[0]));
    console.log(doRight(cpf[1]));
    console.log(doRight(cpf[2]));
    console.log(doRight(cpf[3]));
    ...
    */
    ...
    reg = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'.match(/(<[\w]+>)|([\wáíé\s]+)|(<[\/\w]+>)/g);
    var output = '';
    for (var i = 2; i <= reg.length; i += 3) {
        output += reg[i-2] + 'O texto dentro da tag "' + reg[i-2].replace('<', '').replace('>', '') + '" é "' + reg[i-1] + '"' + reg[i] + '\n';
    }
    console.log(output);
})();
